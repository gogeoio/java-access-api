...
public class App {
  ...
  /**
   * This works just in Java 7 or high. If you are using a lower version comment this method
   * 
   * @return
   */
  ...
  public App() {
	  requestInterceptor = new RequestInterceptor() {
	    ...
	    // Adding gogeo services defined in GogeoService class to the rest Client (retrofit)
  ...
  public void list() {
	  List<Document> docs = service.listDocuments(DATABASENAME, COLLECTIONNAME);
	  System.out.println("[");
	  for(Document d: docs) {
		  System.out.println("{");
   ...
   */
  public void execute() {
    // Define the interceptor, add authentication headers. All request will pass by 'intercept' method.
    

    // Try to create the database, if it exist an error is printed on screem with Error Code 400
    try {
    ...
    for (Database db : databases) {
      System.out.println("\t"+db.getId() + " --> " + db.getDatabaseName());
    }

    // Try to create the collection, if it exist an error is printed on screem with Error Code 400
    try {
    ...
    try {
      service.insertDocument(DATABASENAME, COLLECTIONNAME, document);
      list();
    } catch (Exception e) {
